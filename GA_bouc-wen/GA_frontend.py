...
def make_window(theme):
    ...
    input_layout = [
        ...
        [sg.Text('sol_per_population', size=(20, 1)), sg.Spin(values=[i for i in range(1, 1000)], initial_value=30, size=(6, 1))],
        [sg.Text('mutation_probability', size=(20, 1), pad=((5, 3))), sg.Spin(values=[round(i, 2) for i in seq(0, 1.01, 0.01)], initial_value=0.5, size=(6, 1))],
        [sg.Text('num_generation', size=(20, 1)), sg.Spin(values=[i for i in range(1, 1000)], initial_value=30, size=(6, 1))],
        [sg.Text('crossover_probability', size=(20, 1), pad=((5, 3))), sg.Spin(values=[round(i, 2) for i in seq(0, 1.01, 0.01)], initial_value=0.5, size=(6, 1))],
        [sg.Text('num_parents_mating', size=(20, 1)), sg.Input(default_text='7', size=(8, 1))],
        [sg.Text('crossover_type', size=(20, 1)), sg.Drop(values=('single_point', 'two_points', 'uniform', 'scattered'))],
        [sg.Text('parent_selection_type', size=(20, 1)), sg.Drop(values=('sss', 'rws', 'sus', 'rank', 'random', 'tournament'))],
        [sg.Text('mutation_type', size=(20, 1)), sg.Drop(values=('random', 'swap', 'inversion', 'scramble', 'adaptive'))],
        [sg.Text('keep_parents', size=(20, 1)), sg.Spin(values=[i for i in range(-1, 100)], initial_value=-1, size=(6, 1))],
        [sg.Text('mutation_by_replacement', size=(20, 1)), sg.Drop(values=('False', 'True'))],
        [sg.Text('K_tournament', size=(20, 1)), sg.Spin(values=[i for i in range(1, 100)], initial_value=3, size=(6, 1))],
        [sg.Text('random_mutation_min_val', size=(20, 1)), sg.Input(default_text='-1.0', size=(8, 1))],
        [sg.Text('gene_type', size=(20, 1)), sg.Input(default_text='float', size=(8, 1))],
    ...
    layout = [[sg.MenubarCustom(menu_def, key='-MENU-', font='Courier 15', tearoff=True)],
              [sg.Text('Insert inputs, run program and display your results!', size=(100, 1), justification='center', font=("Helvetica", 16),
...
def main():
    ...
    desired_output_FORCE, function_inputs_DISPLACEMENTS = [], []
    ...
    while True:
        ...
        if event in (None, 'Exit'):
            ...
            if desired_output_FORCE == [] and function_inputs_DISPLACEMENTS == []:
