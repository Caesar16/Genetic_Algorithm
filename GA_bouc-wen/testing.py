...
df = pd.read_csv('bialy_c109-1-full.csv', encoding='unicode_escape')
...
desired_output_FORCE = desired_output_FORCE[:500]
function_inputs_DISPLACEMENTS = df['Przemieszczenie'].to_list()
function_inputs_DISPLACEMENTS = function_inputs_DISPLACEMENTS[:500]

for each in range(len(desired_output_FORCE)):
    desired_output_FORCE[each] = -1 * desired_output_FORCE[each]
    function_inputs_DISPLACEMENTS[each] = -1 * function_inputs_DISPLACEMENTS[each]
...
def fitness_func(solution, solution_idx):
    ...
    for i in range(len(function_inputs_DISPLACEMENTS)):
        if i == 0:
            ...
            z = 1
            force = 0
        if i != (len(function_inputs_DISPLACEMENTS) - 1) and i != 0:
            v = (function_inputs_DISPLACEMENTS[i+1] - function_inputs_DISPLACEMENTS[i]) / delta_t
        if v > 0 and i != 0:
            #print(-gammas, abs(v), abs(z), ns - 1, z, betas, v, abs(z), ns, ast, v)
            z_derivative = -gammas * abs(v) * np.power(abs(z), ns - 1) * z - betas * v * np.power(abs(z), ns) + ast * v
            z = (z_derivative * delta_t) - z
            #print(z_derivative, z)
        elif v <= 0 and i != 0:
            #print(-gammac, abs(v), abs(z), nc - 1, z, betac, v, abs(z), nc, ast, v)
            z_derivative = -gammac * abs(v) * np.power(abs(z), nc - 1) * z - betac * v * np.power(abs(z), nc) + ac * v
            z = (z_derivative * delta_t) + z
            #print(z_derivative, z)
        if v > 0 and i != 0:
            force = (2 * etas * omegas * v + alfas * np.power(omegas, 2) * function_inputs_DISPLACEMENTS[i] + (1 - alfas) * np.power(omegas, 2) * z)
        elif v <= 0 and i != 0:
            force = (2 * etac * omegac * v + alfac * np.power(omegac, 2) * function_inputs_DISPLACEMENTS[i] + (1 - alfac) * np.power(omegac, 2) * z)
        local_cache.append(force)
        temp_fitness += np.abs(float(desired_output_FORCE[i]) - force)
        #print(force)
...
def draw_chart(function_inputs, desired_output, count_output):
    ...
    for each in range(len(function_inputs)):
        if float(count_output[each]) > float(desired_output[each]) and each != 0:
            prc = 100 - ((float(desired_output[each]) / float(count_output[each])) * 100)
        elif float(count_output[each]) <= float(desired_output[each]) and each != 0:
